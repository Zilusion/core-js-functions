...
function getPolynom(...coefficients) {
  ...
  return function polynomial(x) {
    return coefficients.reduce(
      (acc, coeff, index) =>
        acc + coeff * x ** (coefficients.length - index - 1),
      0
    );
  };
}
...
function logger(func, logFunc) {
  return function wrapped(...args) {
    ...
    const logTextStart = `${func.name}(${argsStr}) starts`;
    logFunc(logTextStart);
    const result = func(...args);
    const logTextEnd = `${func.name}(${argsStr}) ends`;
    logFunc(logTextEnd);
    return result;
  };
}
 ...
 *
 * @param {Function} fn
 * @param {...*} args1 - Initial arguments to partially apply
 * @return {Function}
 *
 * @example
...
function partialUsingArguments(fn, ...args1) {
  return function partialUsingArgumentsInner(...args2) {
